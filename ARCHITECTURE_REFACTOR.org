...
*** TODO Add ensure_api_server() function
#+begin_src python
def ensure_api_server(port: int = 9999) -> str:
...
*** TODO CLI creates run via API
#+begin_src python
def create_run(api_url: str, suite_id: int, tests: list) -> str:
    """Create run in DB via API, return run_id."""
    resp = requests.post(f"{api_url}/api/runs", json={
        "suite_id": suite_id,
        "tests": [{"test_id": t.id, "name": t.name, ...} for t in tests]
    })
    return resp.json()["run_id"]
#+end_src

*** TODO Remove RunnerServer class from cli.py
- Delete RunnerServer import and usage
- Replace with ensure_api_server() + API calls

*** TODO Update run_docker_mode()
- Use API server URL instead of RunnerServer URL
- Pass URL to containers via environment variable

*** TODO Update run_local_mode()
- Use API server URL for state/capture/progress
- Same endpoints, just different server
