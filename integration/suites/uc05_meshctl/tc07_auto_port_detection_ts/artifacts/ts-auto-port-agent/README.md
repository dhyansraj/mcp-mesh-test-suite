...
```bash
# Build the image
docker build -t ts-auto-port-agent:latest .

# Run the container
docker run -p 9000:9000 ts-auto-port-agent:latest
```
...
```bash
# Deploy using Helm
helm install ts-auto-port-agent oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  -f helm-values.yaml \
  --set image.repository=your-registry/ts-auto-port-agent \
