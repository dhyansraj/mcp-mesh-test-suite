...
```bash
# Build the image
docker build -t ts-optional-dep-agent:latest .

# Run the container
docker run -p 9023:9023 ts-optional-dep-agent:latest
```
...
```bash
# Deploy using Helm
helm install ts-optional-dep-agent oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  -f helm-values.yaml \
  --set image.repository=your-registry/ts-optional-dep-agent \
