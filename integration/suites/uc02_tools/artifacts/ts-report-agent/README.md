...
```bash
# Build the image
docker build -t ts-report-agent:latest .

# Run the container
docker run -p 9024:9024 ts-report-agent:latest
```
...
```bash
# Deploy using Helm
helm install ts-report-agent oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  -f helm-values.yaml \
  --set image.repository=your-registry/ts-report-agent \
